...
CREATE TABLE departments (
    department_id INT PRIMARY KEY, -- pk_departments
    name VARCHAR(100)
);

CREATE TABLE employees (
    employee_id INT PRIMARY KEY, -- pk_employees
    name VARCHAR(100),
    salary DECIMAL(10,2),
    department_id INT,
...
--Biểu thức hằng số
SELECT name, 'Available' AS status FROM products;

--Biểu thức số học
SELECT product_id, item, price * item AS total FROM order;


--Biểu thức chuỗi
--Ghép chuỗi
SELECT name || ' - ' || category AS full_name FROM products;

--Cắt chuỗi
SELECT SUBSTRING(name FROM 1 FOR 5) AS short_name FROM products;

--Thay thế chuỗi
SELECT REPLACE(name, 'Old', 'New') AS updated_name FROM products;
...
SELECT order_id, delivery_date - order_date AS shipping_days
FROM orders;